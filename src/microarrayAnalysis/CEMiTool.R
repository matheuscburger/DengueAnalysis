...
goWGCNA <- function(data, file_name, corr.method = "spearman"){
    ...
    # determines the soft-threshold and finds the co-expression modules
    D <- t(data)
...
SplitModules <- function(CorMatrix, Gpos, cutPvalue, AorB, mod, geneS, corr.method = "spearman", label) {
    ...
    column.name <- paste0("CEMiTool_", label)
...
# CreateGGplot2GraphFULL  <- function (dataX, geneX) {
#     g	<- graph.edgelist(as.matrix(dataX[, 1:2]), directed = FALSE)
#     layout.used <- layout.fruchterman.reingold(g, niter = 500)
#     my.df 		<- as.data.frame(layout.used) 
#     my.df[, "nodes"] <- unlist(vertex.attributes(g))
#     my.df[, "group"] <- geneX[unlist(my.df$nodes), "NewModule"]
#     my.df[, "CorGrupo"] <- paste("#", geneX[unlist(my.df$nodes), "NewColor"], sep = "")
#     dataX$from.x 	<- my.df$V1[match(dataX[,1], my.df$nodes)] 
#     dataX$from.y 	<- my.df$V2[match(dataX[,1], my.df$nodes)]
#     dataX$to.x 	<- my.df$V1[match(dataX[,2], my.df$nodes)] 
#     dataX$to.y 	<- my.df$V2[match(dataX[,2], my.df$nodes)]
#     Edge_width 	<- (5 + 2*log(abs(dataX[, 3])) - min(5 + 2*log(abs(dataX[, 3]))))/diff(range(5 + 2*log(abs(dataX[, 3])))) + 0.1
#     dataX[dataX[, 3] > 0,"CorLine"] <- 1
#     dataX[dataX[, 3] <= 0,"CorLine"] <- 2
#     colnames(dataX)[1] <- "X"
#     colnames(dataX)[2] <- "Y"
#     dataX$weights      <- Edge_width
#     Numbers 	   <- table(my.df$group)
#     Names   	   <- rownames(Numbers)
#     rects   	   <- data.frame()
#     for (grupo in 1:length(Names))
#     {
#         if (Numbers[grupo] != "0") {
#             minX <- min(my.df[which(my.df$group == Names[grupo]), 1])-(abs(min(my.df[which(my.df$group == Names[grupo]), 1])/10))
#             maxX <- max(my.df[which(my.df$group == Names[grupo]), 1])+(abs(max(my.df[which(my.df$group == Names[grupo]), 1])/10))
#             minY <- min(my.df[which(my.df$group == Names[grupo]), 2])-(abs(min(my.df[which(my.df$group == Names[grupo]), 2])/10))
#             maxY <- max(my.df[which(my.df$group == Names[grupo]), 2])+(abs(max(my.df[which(my.df$group == Names[grupo]), 2])/10))
#             corG <- unique(my.df[which(my.df$group == Names[grupo]),5])
#             row  <- data.frame(Names[grupo], minX, maxX, minY, maxY, corG)
#             rects <- rbind(rects, row)
#         }
...
#     c <- geom_rect(data = rects, aes(xmin = minX, xmax = maxX, ymin = minY, ymax = maxY), 
#                    fill = "blue", alpha = 0, linetype = "dashed", colour = corG)
#     d <- annotate("text", x = (rects[, "maxX"] + rects[, "minX"])/2, y = rects[,"maxY"] + (abs(rects[, "maxY"]/10)),
#                   label = rects[, 1], size = 4)
#     graphplot <- ggplot() +
#         geom_segment(data = dataX,aes (x = from.x, xend = to.x, y = from.y, yend = to.y, 
#                                        width = 0.5), color = "lightgrey", alpha = 0.5) +
#         geom_point(data = my.df,aes(x = V1, y = V2), size = 4, colour = "white") + 
#         geom_point(data = my.df,aes(x = V1, y = V2), size = 2, colour = my.df$CorGrupo, alpha=0.5) +
#         scale_x_continuous(expand = c(0,3)) +
#         scale_y_continuous(expand = c(0,(abs(rects[,"maxY"]/9)))) + 
#         theme_bw() +  c + d +
#         theme(axis.text.x  = element_blank(),  
...
if (!is.null(gmt_f)){
    ...
    gmt <- read.gmt(gmt_f)
    genesets <- gmt[['genes']]
    genes.in.genesets <- unique(unlist(genesets))
...
################################
###Se houver um PPI file
###Representar os grafos apenas usando as PPIs


#if( nrow(exp.df) > genenum || ncol(exp.df) > 400 ){
#    writeLines("Error! Table is too long.", con = paste0(name_out, "_error.txt"), sep = "\n", useBytes = FALSE)
#    stop("Error! Table is too long (maximum dimension: \"genenum\"x400).")
#}
...
write.table(eigen.list, file = paste(name_out, "_eigenlist.txt", sep = ""), sep = "\t", row.names = F, quote = FALSE)
...
new.names <- c(paste0("WGCNA_", name_out), paste0("CEMiTool_", name_out), 
...
subMs   <- rownames(table(out_wgcna[,paste0("CEMiTool_", name_out)])) 

GS <- list()

for (mX in 1:length(subMs)){
    Mgenes <- rownames(out_wgcna[which(out_wgcna[,paste0("CEMiTool_", name_out)]==subMs[mX]),])
...
if(!is.null(gmt_f) & length(gset) > 0){
    ...
    if(sum(rownames(exp.df) %in% gmt.genes) > 0){
        ...
        mod2genes <- split(rownames(out_wgcna), out_wgcna[, paste0("CEMiTool_", name_out)])
        ...
        res.gmt <- lapply(names(mod2genes), function(name){
            ...
            graphColor <- as.character(out_wgcna[match(name, out_wgcna[, paste0("CEMiTool_", name_out)]), "NewColor"])
...
col <- paste("CEMiTool_", name_out, sep="")
