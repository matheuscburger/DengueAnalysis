...
do.ora.list <- function(topgenes, gmt.list, allgenes, ...){
    ...
    if(!is.null(enriched) && !is.logical(enriched)){
        ...
        result <- data.frame(title=character(), direction=character(), Module=character(),
                   ID=character(), Description=character(), GeneRatio=character(),
                   BgRatio=character(), pvalue=character(), p.adjust=character(),
                   qvalue=character(), geneID=character(), Count=character())
    }
    return(result)
}
    ...
    for(cmp in comparisons){
        ...
        for(direction in c("up", "down")){
            ...
            if(!is.null(enriched) && !is.logical(enriched)){
                ...
                result <- data.frame(character(), direction=character(), Module=character(),
                           ID=character(), Description=character(), GeneRatio=character(),
                           BgRatio=character(), pvalue=character(), p.adjust=character(),
                           qvalue=character(), geneID=character(), Count=character())
            }
