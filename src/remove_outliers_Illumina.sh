...
while [ $num_outliers -gt 0 ]; do
	...
	src/microarrayAnalysis/aqm.R --input-exp tmp/${study}.tsv config/sample_annotation/${study}.tsv quality_control/before_norm/${study}_$count --int-cols ExtendedClass --int-cols Class 2> log/preprocess/aqm_before_norm_${study}_$count.txt
	...
	# normaliza
	src/microarrayAnalysis/do_quantile.R tmp/${study}.tsv data/processed/normalized/${study}.tsv --avoid-0 --offset=1 --annotation-cols=ProbeName 2> log/preprocess/quantile_${study}_$count.txt
...
while [ $num_outliers -gt 0 ]; do
	...
	# arrayQualityMetrics depois de normalizar
	src/microarrayAnalysis/aqm.R --input-exp data/processed/normalized/${study}.tsv config/sample_annotation/${study}.tsv quality_control/after_norm/${study}_$count --int-cols ExtendedClass --int-cols Class 2> log/preprocess/aqm_before_norm_${study}_$count.txt
...
echo "remove_outliers_Illumina done."
